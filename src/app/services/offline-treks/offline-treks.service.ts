...
export class OfflineTreksService implements TreksService {
  ...
  public getTrekDetailsUrl(trekId: number): string {
    return `/app/tabs/treks-offline/trek-details/${trekId}`;
  }

  public getTrekMapUrl(trekId: number): string {
    return `/app/map-offline/${trekId}`;
  }
  ...
  public removeTrek(trekId: number, withMedia: boolean): Observable<boolean> {
    ...
    let stream = forkJoin(
      // remove json data
      from(storage.set('offline-treks', JSON.stringify(treks))),
      from(storage.remove(`pois-trek-${trekId}`)),
    ).pipe(map(() => true));
  ...
  public getPoisForTrekById(trekId: number): Observable<Pois> {
    return from(this.storage.get(`pois-trek-${trekId}`)).pipe(
      map((jsonPois: string) => JSON.parse(jsonPois) as Poi[]),
      map(
        (pois: Poi[]) =>
          ({
            type: 'FeatureCollection',
  ...
  public getTouristicContentsForTrekById(trekId: number): Observable<TouristicContents> {
    return from(this.storage.get(`touristicContents-trek-${trekId}`)).pipe(
